...
def get_prediction_metrics(prediction: pd.DataFrame):
    ...
    pm.weighted_detection_rate = sum(
        [(cm[1][1] / (cm[1][1] + cm[1][0])) * class_size if (cm[1][1] + cm[1][0]) != 0 else 0
         for cm, class_size in zip(pm.confusion_matrix, class_sizes)]) / sum(class_sizes)
...
pca = joblib.load("../mymodel/pretreatment-model/pca.pkl")
ss = joblib.load("../mymodel/pretreatment-model/ss.pkl")
norm = joblib.load("../mymodel/pretreatment-model/norm.pkl")
...
test_data, test_label = preproc_data(data)

model = tf.keras.models.load_model("../models/50_classifier_ecgan.h5")
print(model.summary())
prediction = make_prediction(model, test_data, test_label)
print(prediction)
